...
Zuteilung <- function(Dataframe,gruppe, x) {
 ...
 #erstwunsch 
  ...
  helper_2 <- join_all(list(helper_1, helper_group) , by = 'matrikelnummer', type = 'full')
  
  #zweitwunsch
  helper_1 <- subset(helper_2, Platz_erhalten==0)
  ...
  helper_all <- join_all(list(helper_1, helper_all) , by = 'matrikelnummer', type = 'full')
    helper_2 <- join_all(list(helper_1, helper_2) , by = 'matrikelnummer', type = 'full')
...
for (i in 1:10){
  data <-  Zuteilung(data,"m",i)
}
rm(i)
#Staatsexamen
for (i in 1:10){
  data <-  Zuteilung(data,"s",i)
}
rm(i)
